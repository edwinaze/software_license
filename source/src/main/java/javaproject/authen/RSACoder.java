...
public class RSACoder {
    ...
    // 公钥
    private static final String PUBLIC_KEY = "lucy";

    // 私钥
    private static final String PRIVATE_KEY = "david";
    ...
    public static byte[] getAuthencode(byte[] orignal_data, String key) throws Exception{
        byte[] keyy = Base64.getDecoder().decode(key);
        // 取得私钥
        PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(keyy);
        KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
        // 生成私钥
        PrivateKey privateKey = keyFactory.generatePrivate(pkcs8KeySpec);
        // 数据加密
        Cipher cipher = Cipher.getInstance(keyFactory.getAlgorithm());
        cipher.init(Cipher.ENCRYPT_MODE, privateKey);
        return cipher.doFinal(orignal_data);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("甲方 使用乙方公钥加密后的数据：" + Base64.getEncoder().encodeToString(code1));
